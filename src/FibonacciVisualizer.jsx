...
const FibonacciVisualizer = () => {
  ...
  const [inputValue, setInputValue] = useState(5); // Default value for Fibonacci input
  let maxWidth = 0;
  ...
  const fibonacci = (n, x, y, depth, parentIndex = null) => {
    const currentIndex = functionCalls.length;
    functionCalls.push({ x, y, n, depth, parentIndex });

    if (n === 0 || n === 1) {
      recursionCalls.push({ x, y, n, depth, parentIndex, action: 'exit', result: n, baseCase: 'basecase' });
      return n;
    }
    recursionCalls.push({ x, y, n, depth, drawBranch: true, parentIndex, action: 'enter' });

    const horizontalSpacing = maxWidth / (2 ** depth);
    const verticalSpacing = 100;

    const left = fibonacci(n - 1, x - horizontalSpacing, y + verticalSpacing, depth + 1, currentIndex);
    recursionCalls.push({ x, y, n, depth, drawBranch: true, parentIndex, action: 'enter' });

    const right = fibonacci(n - 2, x + horizontalSpacing, y + verticalSpacing, depth + 1, currentIndex);
    recursionCalls.push({ x, y, n, depth, drawBranch: true, parentIndex, action: 'exit', result: left + right });
    return left + right;
  };

  const startVisualization = () => {
    const n = parseInt(inputValue, 10); // Get user input for Fibonacci number
    if (isNaN(n) || n < 0) return; // Validate input
    maxWidth = 120 * n;
    functionCalls = [];
    recursionCalls = [];
    setCompletedSteps([]);
    fibonacci(n, maxWidth + (n * n * n + (100 * n)), 50, 0);
  ...
  const getStatusMessage = () => {
    if (currentStep < finalRecursionCalls.length) {
      ...
      if (currentCall.action === 'enter') {
        return `Calling F(${currentCall.n}) at depth ${currentCall.depth}`;
      } else if (currentCall.action === 'exit') {
        return `Returning F(${currentCall.n}) = ${currentCall.result}`;
      } else if (currentCall.baseCase) {
        return `Base case reached: F(${currentCall.n}) = ${currentCall.result}`;
      }
    }
    return '';
  };
  ...
  return (
    <div className="main">
      <h1>Fibonacci Recursion Visualizer</h1>
      <div >
        <input 
          type="number" 
          value={inputValue} 
      ...
      <div className="visualizer-container" style={{ position: 'relative', width: '100%', height: '600px' }}>
        ...
        {finalRecursionCalls.map((call, index) => {
          ...
          const isBaseCase = call.action === 'basecase';
